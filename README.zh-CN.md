# FOFO Studio

**For Others, For Ourselves**

一个融合 Web3、AIGC 与潮流文化的虚实结合平台。

## 🌍 语言选择

[English](./README.md) (Default) | [中文简体](./README.zh-CN.md) | [日本語](./docs/ja/README.md) | [한국어](./docs/ko/README.md) | [繁體中文](./docs/zh-TW/README.md) | [Español](./docs/es/README.md) | [עברית](./docs/he/README.md) | [Tiếng Việt](./docs/vi/README.md) | [ไทย](./docs/th/README.md) | [Монгол](./docs/mn/README.md) | [Deutsch](./docs/de/README.md) | [Français](./docs/fr/README.md)

---

## 项目简介

FOFO Studio 是一个先锋的"潮流文化交易所"，将数字时代的骑士精神与去中心化的集体主义相结合，为用户提供独一无二的 AI 生成 NFT 实体产品体验。

## 核心特性

- 🤖 **The Neural Mint** - AI 生成 NFT 并实体化
- 🎨 **虚实引擎** - Prompt to Product 协议
- 👥 **Alpha DAO** - 社区共创与版税分享
- 🛍️ **The Market** - 赛博朋克风格电商平台
- 💰 **Web3 支付** - 支持加密货币支付

## 项目结构

```
fofo-studio/
├── README.md              # 项目说明 (English)
├── README.zh-CN.md        # 项目说明 (中文)
├── docs/                  # 文档目录
│   ├── en/                # 英文文档
│   │   ├── README.md
│   │   └── whitepaper.md
│   └── zh-CN/             # 中文文档
│       ├── README.md
│       ├── whitepaper.md
│       ├── architecture.md
│       ├── product-requirements.md
│       └── brand-guidelines.md
└── LICENSE                # 开源协议
```

## 快速开始

### 对于投资者/合作伙伴

👉 阅读 [白皮书](./docs/zh-CN/whitepaper.md) 或 [Whitepaper](./docs/en/whitepaper.md) (English) 了解项目全貌

### 对于开发者

👉 阅读 [架构设计文档](./docs/zh-CN/architecture.md) 了解技术实现

### 对于产品/设计团队

👉 阅读 [产品需求文档](./docs/zh-CN/product-requirements.md) 和 [品牌指南](./docs/zh-CN/brand-guidelines.md)

## 文档导航

### 中文文档

- 📘 [白皮书](./docs/zh-CN/whitepaper.md) - 项目概述与愿景
- 🏗️ [架构设计](./docs/zh-CN/architecture.md) - 技术架构详解
- 📋 [产品需求](./docs/zh-CN/product-requirements.md) - 功能需求文档
- 🎨 [品牌指南](./docs/zh-CN/brand-guidelines.md) - 视觉识别系统
- 📚 [文档中心](./docs/zh-CN/README.md) - 所有文档索引

### English Documentation

- 📘 [Whitepaper](./docs/en/whitepaper.md) - Project overview & vision
- 📚 [Documentation Center](./docs/en/README.md) - All documentation index
